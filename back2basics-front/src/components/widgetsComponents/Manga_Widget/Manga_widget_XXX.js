...
function MangaDexcomponent(props) {
    ...
    return <>
        ...
        <h3>Description      : </h3>
        <h5> {props.manga.description.en.substring(0, 200) + "..."}</h5>

        <img className="Manga_cover" src={`https://uploads.mangadex.org/covers/${props.manga_id}/${props.manga_cover}.256.jpg`} alt={`${props.manga.title.en}`} />
...
export default function MangaDex() {
    ...
    return (
        ...
        <div class="bigWidget">
            <button onClick={RandomMangadex} > Hentaï Randomizer !!! </button>
            {
                DataisLoaded === true ?
                    <MangaDexcomponent
                        manga={manga}
